...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _sendToAI() async {
    if (_textController.text.trim().isEmpty) {
      _showSnackBar('Lütfen haftalık planınızı yazın', Icons.warning_amber_rounded, Colors.orange);
      return;
    }
    ...
    try {
      ...
      if (response.isEmpty) {
        _showSnackBar('Görev bulunamadı, lütfen daha detaylı yazın', Icons.info_outline, Colors.blue);
        return;
      }
  ...
  List<Task> _parseTasksFromAI(String aiResponse) {
    ...
    for (String line in lines) {
      ...
      if (line.contains('Pazartesi') || line.contains('Salı') || 
          line.contains('Çarşamba') || line.contains('Perşembe') ||
          line.contains('Cuma') || line.contains('Cumartesi') || 
          line.contains('Pazar')) {
        ...
        tasks.add(Task(
          ...
          day: currentDay,
          time: time,
          motivationalQuote: motivationalQuotes[tasks.length % motivationalQuotes.length],
        ));
      }
    }
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        gradient: LinearGradient(
                                          colors: [
                                            _getDayColor(index),
                                            _getDayColor(index).withOpacity(0.7),
                                          ],
                                        ),
                                      ),
                                      child: Row(
